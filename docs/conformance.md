...
`tests/rust_control_conformance.rs` permanently runs both the Python-current
backend and the Rust target backend against this fixture. The fixture declares
the two intended profile differences below. Any other response or external
effect-count difference fails the same Cargo test.
